...
{{-- Pake layout admin sebagai kerangka halaman --}}

@section('content')
{{-- Bagian konten utama halaman admin --}}

    <h2 class="text-2xl font-bold mb-4">Laporan Masuk</h2>
    {{-- Judul halaman --}}

    @if(session('success'))
        {{-- Tampilkan notifikasi kalau ada pesan sukses dari session --}}
    ...
    <div class="overflow-x-auto">
        {{-- Bikin tabel scrollable kalau lebarnya melebihi layar --}}
            ...
            <tbody class="bg-white dark:bg-[#181818]">
                ...
                @forelse($reports as $report)
                    <tr class="border-b border-gray-300 dark:border-[#3a3a3a]">
                        ...
                        <td class="px-4 py-2">
                            {{-- Cek apakah laporan anonim atau tidak --}}
                        ...
                        {{-- Format tanggal laporan dibuat --}}
                        <td class="px-4 py-2">{{ $report->created_at->format('d M Y') }}</td>
                        <td class="px-4 py-2">
                            {{-- Form buat update status laporan langsung dari dropdown --}}
                ...
                @empty
                    {{-- Kalau belum ada laporan, tampilkan pesan ini --}}
                    <tr>
